    ...
    with Writer(psycopg_cursor, "public.e2e_test_e2e_test_1") as writer:
        with Encoder(schema, writer) as encoder:
            encoder.append_tuple(
                (1, 2, 3, 1.23, Decimal("2.2622600000000003164"), "1", "-1", date(2019, 1, 1), '{"value": 1}', b'{"value": -1}')
            )
            encoder.append_tuple(
                (10, 20, 30, 12.3, Decimal("-5.69483"), "10", "-10", date(2019, 1, 2), '{"value": 10}', b'{"value": -10}')
            )

    with WriterEncoder(psycopg_cursor, "public.e2e_test_e2e_test_1", schema) as writer_encoder:
        writer_encoder.append_tuple(
            (2, 3, 4, 2.34, Decimal("7890"), "2", "-2", date(2019, 2, 1), '{"value": 2}', b'{"value": -2}')
        )
        writer_encoder.append_tuple(
            (20, 30, 40, 23.4, Decimal("-0"), "20", "-20", date(2019, 2, 2), '{"value": 20}', b'{"value": -20}')
        )
